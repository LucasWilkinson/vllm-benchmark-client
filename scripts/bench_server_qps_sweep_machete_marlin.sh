...
do
    ...
    vllm serve $MODEL_NAME \
        ...
        --port $PORT $OPTIONS \
        -tp $TP \
        --disable-async-output-proc &> vllm_server_marlin_$NAME.log &
    SERVER_PID=$!
    echo "**** Server PID: $SERVER_PID"
    echo "**** Waiting for server to start, server output redirected to vllm_server_marlin_$NAME.log"
    
    tail -f vllm_server_marlin_$NAME.log | while read LOGLINE
    do
        [[ "${LOGLINE}" == *"Uvicorn running on http://0.0.0.0:$PORT"* ]] && pkill -P $$ tail
    done
    ...
    python3 benchmark/benchmark_serving_qps_sweep.py \
        ...
        --dataset $DATASET \
        --query-issue-time $TIME \
        --outfile $DATASET-$NAME-marlin-$OUTPUT_SUFFIX.json \
        --qps 0.1 1 3 5 8 10
    done
    kill $SERVER_PID
    fuser -k $PORT/tcp

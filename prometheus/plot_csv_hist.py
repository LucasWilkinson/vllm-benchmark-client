...
def plot_hist_df(df, ax, scale_y_factor=1.0, xlabels_rotation=0, buckets=None):
    ...
    if buckets:
        ...
        def bucket_start_end(s):
            return [int(x) if x != "+Inf" else math.inf for x in s.split("-")]
...
if __name__ == "__main__":
    ...
    parser.add_argument("csvs",
                        type=str,
                        help="Path to the CSV result files to plot.",
                        nargs='+')
    parser.add_argument(
        "--outfile",
        type=str,
    ...
    fig, axs = plt.subplots(len(args.csvs),
                            1,
    ...
    for i, csv in enumerate(args.csvs):
        ...
        plot_hist_df(df,
                     ...
                     buckets=[b.split(":")
                              for b in args.buckets] if args.buckets else None)

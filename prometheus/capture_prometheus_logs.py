...
def plot_hist(prom: PrometheusClient, metric_str: str, time_window_expr: str, include_inf=True, ax=None, normalize=False):
...
def plot_iter_tokens_hist(prom: PrometheusClient, time_window_expr: str, **kwargs):
    plot_hist(prom, "vllm:iteration_tokens_total_bucket", time_window_expr, include_inf=False, **kwargs)

def plot_iter_tpot_hist(prom: PrometheusClient, time_window_expr: str, **kwargs):
    plot_hist(prom, "vllm:time_per_output_token_seconds_bucket", time_window_expr, include_inf=True, **kwargs)

def plot_iter_ttft_hist(prom: PrometheusClient, time_window_expr: str, **kwargs):
    plot_hist(prom, "vllm:time_to_first_token_seconds_bucket", time_window_expr, **kwargs)

def plot_iter_tokens_sum_count(prom: PrometheusClient, time_window_expr: str, ax=None):    
    total_sum_results = prom.get_metric("vllm:iteration_tokens_total_sum", time_window_expr)
    total_count_results = prom.get_metric("vllm:iteration_tokens_total_count", time_window_expr)
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(4, figsize=(15, 10), sharex=True)  # Adjusting figsize for better layout

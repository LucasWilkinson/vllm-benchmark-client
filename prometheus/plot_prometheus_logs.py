...
def plot_iter_tokens_hist(prom: PrometheusClient, time_window_expr: str,
                          **kwargs):
    plot_hist(
        prom,
...
def plot_iter_tpot_hist(prom: PrometheusClient, time_window_expr: str,
                        **kwargs):
    plot_hist(
        prom,
...
def plot_iter_ttft_hist(prom: PrometheusClient, time_window_expr: str,
                        **kwargs):
    plot_hist(prom, "vllm:time_to_first_token_seconds_bucket",
              time_window_expr, **kwargs)


def plot_iter_tokens_sum_count(prom: PrometheusClient,
                               time_window_expr: str,
                               ax=None):
    total_sum_results = prom.get_metric("vllm:iteration_tokens_total_sum",
                                        time_window_expr)
    total_count_results = prom.get_metric("vllm:iteration_tokens_total_count",
                                          time_window_expr)
    ...
    df_dict = {
        "total_tokens": {t: int(v)
                         for t, v in total_sum_results[0]["values"]},
        "total_iters":
        {t: int(v)
         for t, v in total_count_results[0]["values"]},
    }
